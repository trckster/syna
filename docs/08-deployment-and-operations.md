...
| `SYNA_MAX_WORKSPACES` | maximum number of workspace keys this server may create, default `10`; `0` means unlimited |
...
- environment variable: `SYNA_PUBLIC_BASE_URL=$COOLIFY_URL` with `Literal`
  ...
  start
- optional environment variable: `SYNA_MAX_WORKSPACES=10` for the default
  hosted workspace limit; set a different finite value if needed, or `0` only when
  unlimited workspace creation is intended
- Ports Exposes: `8080`
- Port Mappings: empty
