...
func (db *DB) QueueInitialRootRecovery(rootID string, createdSeq int64, createdAt time.Time) error {
	var existing string
	err := db.SQL.QueryRow(`
		SELECT op_id
		FROM pending_ops
		WHERE root_id = ? AND rel_path = '' AND op_type = 'recover_initial_root'
		ORDER BY created_at ASC
		LIMIT 1
	`, rootID).Scan(&existing)
	switch {
	case errors.Is(err, sql.ErrNoRows):
		_, err = db.SQL.Exec(`
			INSERT INTO pending_ops (op_id, root_id, rel_path, op_type, base_seq, payload_json, status, retry_count, last_error, next_retry_at, created_at)
			VALUES (?, ?, '', 'recover_initial_root', ?, '{}', 'queued', 0, NULL, NULL, ?)
		`, pendingOpID(rootID, createdAt), rootID, createdSeq, createdAt)
		return err
	case err != nil:
		return err
	default:
		_, err = db.SQL.Exec(`UPDATE pending_ops SET base_seq = ?, status = 'queued', next_retry_at = NULL WHERE op_id = ?`, createdSeq, existing)
		return err
	}
}
